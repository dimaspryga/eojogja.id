...
const Footer = () => {
  ...
  return (
    <footer className="text-white bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            {isLoading
              ? Array.from({ length: 7 }).map((_, index) => (
                  <Skeleton
                    key={index}
                    className="w-16 h-8 bg-blue-700 rounded-lg"
                  />
                ))
              : (paymentMethod || []).map((partner) => (
                  ...
                  >
                    <img
                      ...
                      alt={partner.name}
                      className="object-contain h-6 transition-opacity duration-200 sm:h-8 filter brightness-0 invert opacity-80 hover:opacity-100"
                      title={partner.name}
                    />
                  </div>
                ))}
