...
const ActivityCard = ({ item }) => {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden transition-all duration-300 bg-white border border-gray-200 group rounded-xl hover:border-blue-300 hover:-translate-y-1"
    >
            ...
            <div className="flex flex-col">
              {hasDiscount ? (
                <>
                  ...
                  <p className="text-sm font-bold text-red-600 sm:text-lg">
              ...
              <p className="text-xs text-gray-500 sm:text-sm">per person</p>
            </div>
            <Button
              type="button"
              size="sm"
              ...
              <Link href={`/activity/${item.id}`}>
                <div className="relative h-[50vh] w-full rounded-3xl overflow-hidden group shadow-2xl">
                  ...
                  <div className="absolute bottom-0 left-0 p-8 text-white">
                    ...
                    <h2 className="mb-3 text-2xl font-bold drop-shadow-lg sm:text-3xl md:text-4xl">
                      {item.title}
                    </h2>
          ...
          <div className="mb-8 space-y-4 lg:space-y-0 lg:flex lg:items-center lg:space-x-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full py-3 pl-10 pr-4 text-base transition-all duration-300 border border-gray-200 rounded-xl bg-white/80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover:border-gray-300 hover:bg-white/90"
                value={searchQuery}
            ...
            <div className="relative">
              <select
                ...
                className="w-full px-4 py-3 text-base transition-all duration-300 border border-gray-200 appearance-none cursor-pointer lg:w-48 rounded-xl bg-white/80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover:border-gray-300 hover:bg-white/90"
              >
                <option value="all">
                  All Categories ({activity?.length || 0})
                </option>
                {categories.map((cat) => (
                  <option key={cat} value={cat}>
                    {cat} (
                    {activity?.filter(
                      (item) => (item.category?.name || item.category) === cat
                    ).length || 0}
                    )
                  </option>
                ))}
